...
- 2026-07-20 实际验收（无头执行）：`RunAllTests` 502/502、TC17 81/81、TC18 6/6；批量回归 9 计划 13 子批次 661/661 断言通过。
- 2026-07-20 对抗性端到端（生产工作簿）：S1~S8 全通过——正常业务流、重跑覆盖、E01 拦截、空输入、详细调试日志、500 物流单号性能基线、E02 原始值前导零、E06 进异常明细。
- 2026-07-19 项目首次完成闭环（491/659）；详见 `规则-TC-模块-数据集映射表.md` §5.4 执行记录。
...
| `无头业务验证.ps1` | 对生产工作簿做对抗性端到端验证（S1~S8 场景） |
| `一次性脚本/` | 2026-07 收尾阶段的一次性资产/补丁脚本归档（来历凭证，日常勿运行） |
...
- `无头业务验证.ps1`：在临时副本上对生产工作簿跑 S1~S8 对抗性场景（正常流、重跑覆盖、E01 拦截、空输入、详细调试日志、500 单性能基线、E02 原始值前导零、E06 进异常明细）。
...
- 项目已纳入 Git 并推送至 GitHub 私有仓库 `Finnttt/returned_inventory_allocation`（main 分支，2026-07-20）；日常修改后直接 `git add` + `git commit` + `git push` 即可同步远端。
- 修改会写入 Excel 的脚本前，仍须先关闭目标工作簿（脚本已内置占用检测与自动备份）。
